const { kelas_detail, kelas_materi, kelas_benefit, kelas_mentor, kelas_rating, kelas_regist,  } = require('../models');
const { kelas_bisnis, kelas_kategori, kelas_level, kelas_diskon , sequelize, User} = require('../models');
...
module.exports = {
    ...
    getKelasBisnis: async (req, res) => {
        try {
            ...
            let intKategori = kategori ? kategori.split(',').map(Number) : null;
            let intLevel = level ? level.split(',').map(Number) : null;

            const result = await kelas_bisnis.findAll({
                attributes: ['id', 'nama', 'image', 'images_link', 'harga',
                    [sequelize.literal('ROUND(COALESCE(SUM(kelas_ratings.nilai), 0) / COUNT(DISTINCT kelas_ratings.id_user),1)'), 'rata_nilai'],
                    [sequelize.fn('COUNT', sequelize.fn('DISTINCT', sequelize.col('kelas_ratings.id_user'))), 'jumlah_penilai'],
                    [sequelize.literal('(SELECT COUNT(*) FROM kelas_regist WHERE kelas_regist.id_kelas_bisnis = kelas_bisnis.id)'), 'jumlah_pendaftar'],
                ],
                include: [
                ...
                where: {
                    ...(kategori ? { '$kelas_kategori.id$': {[Op.in]: intKategori} } : {}),
                    ...(level ? { '$kelas_level.id$': {[Op.in]: intLevel} } : {}),
                    ...(harga1 && harga2 ? { harga: { [Op.between]: [harga1, harga2] }} : {}),
                },
    ...
    getDetailKelasBisnis: async (req, res) => {
        try {
            ...
            const result = await kelas_detail.findAll({
                ...
                include: [
                    { model: kelas_materi, attributes: ['materi','link','deskripsi'], through: { attributes: [] } },
                    { model: kelas_benefit, attributes: ['benefit','image','images_link','deskripsi'], through: { attributes: [] } },
                    { model: kelas_mentor, attributes: ['nama', 'jabatan', 'perusahaan', 'deskripsi', 'image', 'images_link'], through: { attributes: [] } },
                    {
                        ...
                        include:[
                            {
                                model: User,
                                attributes: ['nama_user','profile_picture','picture_link','nama_bisnis'],
                                required:false
                            }
                        ]
                    },
