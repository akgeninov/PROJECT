...
function DataKarirCard({ cardsData, index }) {
  ...
  if (!cardsData || !Array.isArray(cardsData.data)) {
    // console.error("Data is not valid:", cardsData);
    return null; // or display an error message
  }
  return (
    <div className="flex flex-col gap-5 ">
      {cardsData.data.map((data) => {
        const namaDepartemen = departemen.find(dep => dep.id === data.id_departemen_pekerjaan)?.nama_departemen_pekerjaan || 'Nama Departemen Tidak Ditemukan';
